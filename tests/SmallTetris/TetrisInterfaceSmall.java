...
public class TetrisInterfaceSmall extends Game
{

    private State _state;
    private boolean _visualise_game = false;

    public TetrisInterfaceSmall()
    {
    ...
    {
        int[] init_state = new int[State.ROWS* State.COLS];
        Arrays.fill(init_state, 0);
        init_state[State.ROWS* State.COLS - 1] = _state.getNextPiece();
    ...
    public Game restart()
    {
        TetrisInterfaceSmall new_game = new TetrisInterfaceSmall();
    ...
    public int[] state()
    {
        ...
        int[] _state_arr = new int[height*width]; //4=pos_stone+height+width+numberofstone
        for (int h=0; h<height; h++){
            for (int w=0; w<width; w++){
                _state_arr[h*width+w]= (field[h][w]!=0) ? 1 : 0;
            }
        }
        return _state_arr;
    }
    ...
    public int numStates(){ return (int)Math.pow(2, State.ROWS*State.COLS); }

    public int numActions(){ return State.COLS; }
